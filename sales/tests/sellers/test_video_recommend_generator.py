...
@patch("google.cloud.storage.Client")
def test_read_analysis_invalid_gcs_uri(mock_storage_client, recommendation_generator):
    ...
    with pytest.raises(ValueError, match="Invalid GCS URI format"):
        recommendation_generator.read_analysis_from_gcs("gs://test-bucket/file.mp4")


def test_create_recommendation_prompt(recommendation_generator, sample_analysis_results):
    """Test creation of recommendation prompt."""
    prompt = recommendation_generator._create_recommendation_prompt(sample_analysis_results)
...
def test_create_recommendation_prompt_no_vertical_data(recommendation_generator, sample_analysis_results):
    ...
    # Call the method
    prompt = recommendation_generator._create_recommendation_prompt(analysis_without_vertical)
...
def test_create_recommendation_prompt_empty_data(recommendation_generator):
    ...
    # Call the method
    prompt = recommendation_generator._create_recommendation_prompt(empty_analysis)
...
def test_full_recommendation_workflow(recommendation_generator, sample_analysis_results):
    ...
    blob_mock.download_as_string.return_value = json.dumps(sample_analysis_results).encode('utf-8')
    
    # Mock the Gemini client
    with patch.object(recommendation_generator, 'gemini_client') as mock_gemini:
        ...
        recommendations = recommendation_generator.generate_recommendations(analysis)
