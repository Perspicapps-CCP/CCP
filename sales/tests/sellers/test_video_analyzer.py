...
def mock_annotation_result(
    sample_object_annotations,
    sample_label_annotations,
    sample_text_annotations,
    sample_logo_annotations,
):
...
def test_process_object_annotations(
    video_analyzer, sample_object_annotations
):
    """Test processing of object annotations."""
    processed = video_analyzer._process_object_annotations(
        sample_object_annotations
    )
...
def test_process_label_annotations(video_analyzer, sample_label_annotations):
    ...
    processed = video_analyzer._process_label_annotations(
        sample_label_annotations
    )
...
def test_process_text_annotations(video_analyzer, sample_text_annotations):
    ...
    processed = video_analyzer._process_text_annotations(
        sample_text_annotations
    )
...
def test_process_logo_annotations(video_analyzer, sample_logo_annotations):
    ...
    processed = video_analyzer._process_logo_annotations(
        sample_logo_annotations
    )
...
def test_analyze_vertical_distribution(
    video_analyzer, sample_object_annotations
):
    """Test analysis of vertical distribution."""
    distribution = video_analyzer.analyze_vertical_distribution(
        sample_object_annotations
    )
...
def test_calculate_vertical_distribution_metrics(video_analyzer):
    ...
    level_distribution = {
        "Nivel Superior": {
            "total_objects": 10,
            "objects": {"soda": 5, "chips": 5},
        },
        "Nivel Medio": {"total_objects": 5, "objects": {"candy": 5}},
        "Nivel Inferior": {
            "total_objects": 15,
            "objects": {"water": 8, "juice": 7},
        },
    }

    metrics = video_analyzer._calculate_vertical_distribution_metrics(
        level_distribution
    )
...
def test_analyze_video(
    mock_video_client, video_analyzer, mock_annotation_result
):
...
def test_save_analysis_to_gcs(mock_storage_client, video_analyzer):
    ...
    # Call the method
    result_path = video_analyzer.save_analysis_to_gcs(
        analysis_results, video_path
    )

    # Verify the result
    assert (
        result_path == "gs://test-bucket/videos/analysis_test-video.mp4.json"
    )

    # Check that the storage client was used correctly
    video_analyzer.storage_client.bucket.assert_called_once_with(
        "test-bucket"
    )
    bucket_mock.blob.assert_called_once_with(
        "videos/analysis_test-video.mp4.json"
    )
    blob_mock.upload_from_string.assert_called_once()
    # Check the JSON was formatted correctly
    assert json.loads(blob_mock.upload_from_string.call_args[0][0]) == {
        "test": "data"
    }
...
def test_save_analysis_invalid_path(video_analyzer):
    ...
    with pytest.raises(ValueError):
        video_analyzer.save_analysis_to_gcs(
            {"test": "data"}, "invalid/path.mp4"
        )


@patch("google.cloud.videointelligence.VideoIntelligenceServiceClient")
def test_analyze_video_with_custom_features(
    mock_video_client, video_analyzer, mock_annotation_result
):
    ...
    video_analyzer.analyze_video(
        "gs://test-bucket/test-video.mp4", features=custom_features
    )
...
def test_analyze_vertical_distribution_levels(
    video_analyzer, sample_object_annotations
):
    ...
    for level_name in expected_level_names:
        assert (
            level_name in levels
        ), f"Expected level '{level_name}' not found in results"

    # Check that there are no extra levels
    assert (
        len(levels) == 3
    ), f"Expected exactly 3 levels, but got {len(levels)}"
