...
def create_warehouse(request: schemas.WarehouseSchema, db: Session = Depends(get_db)):
    request = services.create_warehouse(db=db, warehouse=request)
    return mappers.warehouse_created_to_shema(request)


@warehouse_router.get("/", response_model=List[schemas.WarehouseGetResponseSchema])
def list_warehouses(
    params: schemas.ParamsRequest = Depends(), db: Session = Depends(get_db)
):
...
@warehouse_router.get(
    "/{warehouse_id}", response_model=schemas.WarehouseGetResponseSchema
)
def get_warehouse(warehouse_id: str, db: Session = Depends(get_db)):
    warehouse = services.get_warehouse(db, warehouse_id=warehouse_id)
