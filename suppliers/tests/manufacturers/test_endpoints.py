...
def products_ids_payload() -> Dict[str, List[str]]:
    ...
    """
    return {
       "productsIds":["98765549-5fab-40bd-8076-65a05e62c808"]
    }
...
def test_list_products_by_ids(client: TestClient, products_ids_payload: Dict) -> None:
    """
    Test listing products given ids list.
    """
    response = client.post("/suppliers/manufacturers/listProducts",  json=products_ids_payload)
    assert response.status_code == 200
    assert isinstance(response.json(), list)
...
def test_get_product_by_manufacturer_id(
    ...
    create_response = client.post("/suppliers/manufacturers/", json=manufacturer_payload)
    manufacturer_id = create_response.json()["id"]
    response = client.get(f"/suppliers/manufacturers/{manufacturer_id}/products")
    assert response.status_code == 200
